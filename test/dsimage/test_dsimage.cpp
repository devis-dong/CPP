 ...
 * @LastEditTime: 2021-07-20 22:25:07
...
int main()
{
    ...
    GaussPyr<unsigned char> pyr = create_gauss_pyramid<unsigned char>(img_ori, octvs, intvs, sigma);
    // vector<vector<Image<unsigned char>>> pyr = create_gauss_pyramid<unsigned char>(img_ori, octvs, intvs, sigma);
    ...
    {
        for(int j = 0; j < pyr.intvs; ++j)
        {
            string winname = "pyr[";
            winname = winname + to_string(i) + "," + to_string(j) + "]";
            show_image(pyr.imgs[i][j], winname);
        }
    }
    destroy_gauss_pyr(pyr);
    wait_key();
    close_all();
    printf("...done.\n");
    system("pause");
